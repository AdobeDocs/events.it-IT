---
title: Utilizzo di JSON con Workfront Fusion + JSONata
description: Questa sessione ha descritto le nozioni di base sulla lettura della notazione del percorso JSON, sulla comprensione dei tipi di dati e sulla scelta del percorso iniziale corretto in Fusion.
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3618
last-substantial-update: 2025-12-03T00:00:00Z
jira: KT-19780
source-git-commit: 2a5336b19f61bb0e1c48311b36c76c4a0967c410
workflow-type: tm+mt
source-wordcount: '171'
ht-degree: 0%

---


# Utilizzo di JSON con Workfront Fusion + JSONata

Ospitata in collaborazione con Steve Watts e Brian Haight di Adobe Professional Services, questa sessione ha esaminato le nozioni di base sulla lettura della notazione del percorso JSON, la comprensione dei tipi di dati e la scelta del percorso iniziale corretto in Fusion.

Inoltre, sono stati trattati tre esempi reali: trovare e filtrare i dati, rimodellare il JSON e eseguire al volo trasformazioni avanzate. Si tratta di una competenza imprescindibile per qualsiasi amministratore di Fusion che desidera risparmiare tempo e semplificare la gestione dei dati.

>[!VIDEO](https://video.tv.adobe.com/v/3477129/?learn=on&enablevpops)

Se non sei riuscito a partecipare in diretta, non preoccuparti: controlla la [presentazione](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/Slide+Deck+-+JSONata+Fusion+Event+-+September+17%2C+2025.pdf) e guarda la registrazione on-demand per seguirla.

Consulta il [post della community Experience League](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-making-json-sing-with-workfront-fusion-jsonata/td-p/776029?profile.language=it) per trovare un file scaricabile e segui il file zip dei materiali di formazione.

I nuovi eventi vengono aggiunti ogni mese, quindi assicurati di controllare la [pagina Eventi di Experience League](https://experienceleague.adobe.com/it/events) per le ultime sessioni.

